...
const sign = require('../../utils/weixinSign.js'); //微信提供在开发文档可以找到下载到本地
const weChatConfig = require('../../config').weChat;
const base = {
  appid: weChatConfig.appid, //公众号appid
  secret: weChatConfig.secret, // 公众号secret （重要不要暴露在前端）
...
function getAccessToken() {
  return new Promise((resolve, reject) => {
    request.get(
      `${base.wxapi}/token?grant_type=client_credential&appid=${base.appid}&secret=${base.secret}`,
...
function getTicket(access_token) {
  return new Promise((resolve, reject) => {
    request.get(
      `${base.wxapi}/ticket/getticket?access_token=${access_token}&type=jsapi`,
      function(error, response, body) {
        if (error !== null) {
